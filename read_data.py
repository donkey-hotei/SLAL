from core.Vector import Vectort
from core.matrix import Matrix 

def read_Vectortors(filename):
    """File should have the following format:
    First line should consist of labels, separated by tabs or spaces.
    Remaining lines should consist of numeric data.
    Procedure returns a list of Vectortors, one for each line of numeric data.
    The labels for the Vectortors are the strings given in the first line.
    """
    with open(filename) as file:
        labels = file.readline().split()
        vlist = [Vector(set(labels), dict(zip(labels, map(float, line.split())))) for line in file]
        return vlist
